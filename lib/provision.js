...
/** Downloads the Outline installer, so the target server never has to fetch it. */
 ...
 * caller. A host key that has *changed* is never a question.
 */
function connect({
    ...
    return new Promise((resolve, reject) => {
        ...
        client
            ...
            .connect({
                ...
                // server that cannot prove it is the one we mean.
                hostVerifier: key => {
                    ...
                    if (!onUnknownHost({ hostPort, fingerprint: actual })) {
